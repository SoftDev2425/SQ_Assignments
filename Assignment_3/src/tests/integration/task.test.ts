...
describe("Task", () => {
  ...
  test("should update a task", async () => {
    ...
    // Act
    const response = await supertest(app).put(`/api/tasks/${testTask.id}`).send(updatedTask);
  ...
  test("should throw error if title is missing", async () => {
    ...
    expect(response.body).toEqual({ error: "Please provide a title for the task" });
  });
  ...
  test("should handle update task status", async () => {
    ...
    // Act
    const response = await supertest(app).put(`/api/tasks/${testTask.id}/status`).send({ completed: true });
